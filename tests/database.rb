...
class TestDatabase < Test::Unit::TestCase
  ...
  #   end
    ...
    def test_international_place
      ...
      [ {:city=>"Kabul", :state=>"AF", :precision=>:zip, :fips_county=>"17031", :score => 0.857, :lat => 34.5166667},
        {:city=>"Paris", :state=>"FR", :precision=>:zip, :fips_county=>"42101", :score => 0.857}
      ].each {|record|
        result = @db_intl.geocode(record[:city] + ", " + record[:state])
        puts "--lat--"
        puts result
        assert_equal result.length, 1
        record.keys.each {|key| assert_equal record[key], result[0][key]}
